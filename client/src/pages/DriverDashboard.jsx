...
//import { getMyDriverInfo, toggleDriver2FA } from '../api/driverService';
import { getMyDriverInfo, requestDriver2FAToggle, verifyDriver2FA } from '../api/driverService';
...
function PageProfile({ navigate, driverInfo: propInfo }) {
  ...
  useEffect(() => {
  if (propInfo) {
    setDriverInfo(propInfo);
    setTwoFAEnabled(propInfo.twoFA || false);  // ← ADD THIS LINE
    setLoading(false);
  }
}, [propInfo]);
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                    {twoFAEnabled ? 'Your account has extra protection.' : 'Protect your account with 2FA'}
                  </div>
                </div>
...
function Modal2FAVerifyDriver({ action, email, onClose, onSuccess, showToast }) {
  ...
  const handleVerify = async () => {
    ...
    setError('');
    try {
      const res = await verifyDriver2FA(otp);
