...
// When "remember" is false → sessionStorage (cleared when tab/browser closes)

export function saveSession(token, remember = false) {
  const storage = remember ? localStorage : sessionStorage;
  storage.setItem('admin_token', token);
  // Also store which storage was used so we can read it back correctly
  localStorage.setItem('admin_remember', remember ? '1' : '0');
}

export function getSession() {
  const remember = localStorage.getItem('admin_remember') === '1';
  const storage = remember ? localStorage : sessionStorage;
