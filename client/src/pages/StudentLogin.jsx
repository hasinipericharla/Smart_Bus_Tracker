...
// //   // ── OTP expiry timer (forgot password) ──────────────────────────────────
// //   useEffect(() => {
...
// //   const closeModal = () => {
// //     setShowForgot(false);
// //     setFpStep('email'); setFpEmail(''); setFpError(''); setFpSuccess('');
// //     setFpOtpDigits(['', '', '', '', '', '']);
...
// //     // ── DEV: log OTP ── replace with your API call when backend is ready ──
...
// //     setFpError('');
// //     // ── DEV: replace with real API call ───────────────────────────────────
...
// //   // ── login handler ────────────────────────────────────────────────────────
// //   const handleLogin = () => {
// //     if (!studentId || !password) { setError('Please enter both Student ID and Password.'); return; }
// //     setError('');
// //     // ── DEV: replace with real auth call ──────────────────────────────────
// //     // await fetch('/api/login', { method: 'POST', body: JSON.stringify({ studentId, password }) })
...
// //                 <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#1B2B4B', marginBottom: '0.3rem' }}>
// //                   Forgot Password?
// //                 </h3>
...
// //                 </p>
// //                 {fpError && <FpErrorBanner message={fpError} />}
// //                 <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
// //                   <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
// //                     <label htmlFor="fpEmail" style={labelStyle}>Email Address</label>
// //                     <div style={{ position: 'relative' }}>
// //                       <input id="fpEmail" type="email" placeholder="student@college.edu"
...
// //                     </div>
// //                   </div>
// //                   <ModalPrimaryButton onClick={handleFpSendOtp}>Send OTP</ModalPrimaryButton>
// //                 </div>
// //               </>
// //             )}
...
// //                   Check Your Email
// //                 </h3>
// //                 <p style={{ fontSize: '13px', color: '#8898A9', fontWeight: '600', marginBottom: '0.3rem', textAlign: 'center' }}>
// //                   We sent a reset code to
// //                 </p>
// //                 <p style={{ fontSize: '13px', fontWeight: '800', color: '#1B2B4B', marginBottom: '1.4rem' }}>
// //                   {fpEmail}
// //                 </p>
// //                 {fpError && <FpErrorBanner message={fpError} />}
// //                 <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
...
// //                   <ModalPrimaryButton onClick={handleFpVerifyOtp}>Verify Code</ModalPrimaryButton>
...
// //                   <button onClick={() => { setFpStep('email'); setFpError(''); }} style={{ background: 'none', border: 'none', fontSize: '12px', color: '#8898A9', cursor: 'pointer', fontFamily: "'Nunito', sans-serif" }}>
...
// //                     </div>
// //                   </div>
// //                   <ModalPrimaryButton onClick={handleFpReset}>Reset Password</ModalPrimaryButton>
// //                 </div>
// //               </>
// //             )}
...
// //                 <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#1B2B4B', marginBottom: '0.5rem' }}>
// //                   Password Reset!
// //                 </h3>
// //                 <p style={{ fontSize: '13px', color: '#8898A9', fontWeight: '600' }}>
// //                   Your password has been updated. Closing…
// //                 </p>
// //               </div>
// //             )}
// //           </div>
...
// //           <div style={{
// //             background: '#E8EDF5', color: '#2D4A7A', borderRadius: '50px', padding: '6px 18px',
...
// //             Sign in to track your bus & view schedules
// //           </p>
...
// //                   fontSize: '15px', color: '#8898A9', cursor: 'pointer', userSelect: 'none',
// //                 }}>{showPassword ? '🙈' : '👁️'}</span>
// //               </div>
// //             </div>
...
// //                 color: '#1B2B4B', border: '1.5px solid #D8E2EE', borderRadius: '12px',
...
// // function FpErrorBanner({ message }) {
// //   return (
...
// // function ModalPrimaryButton({ onClick, children }) {
// //   return (
// //     <button
// //       onClick={onClick}
// //       onMouseEnter={e => e.currentTarget.style.background = '#F5A623'}
...
//   const handleLogin = () => {
//     if (!identifier || !password) { setError('Please enter your Student ID / Email and Password.'); return; }
//     setError('');
//     navigate('/student/dashboard');
//   };
...
//         <h2 style={{ fontSize: '21px', fontWeight: '800', color: '#1B2B4B', marginBottom: '0.3rem' }}>Welcome back, Student</h2>
//         <p style={{ fontSize: '13px', color: '#8898A9', fontWeight: '600', marginBottom: '1.8rem', textAlign: 'center' }}>Sign in to track your bus & view schedules</p>
...
//           {/* Student ID / Email */}
//           <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
//             <label htmlFor="identifier" style={labelStyle}>Student ID / Email</label>
//             <div style={{ position: 'relative' }}>
//               <input id="identifier" type="text" placeholder="e.g. STU2024001 or student@college.edu"
...
const inputStyle = {
  ...
  border: '1.5px solid #D8E2EE', borderRadius: '10px',
  fontSize: '14px', fontFamily: "'Nunito', sans-serif",
  color: '#1B2B4B', background: '#F7FAFD', outline: 'none', boxSizing: 'border-box',
};
const focusStyle = (e) => { e.target.style.borderColor = '#F5A623'; e.target.style.boxShadow = '0 0 0 3px rgba(245,166,35,0.12)'; e.target.style.background = '#fff'; };
...
export default function StudentLogin() {
  ...
  // Unverified account: redirect back to signup OTP step
  ...
  // ── FORGOT: step 1 — send OTP → real API ─────────────────────────────────
  const handleFpSendOtp = async () => {
    if (!fpEmail) return setFpError('Please enter your email address.');
    if (!/\S+@\S+\.\S+/.test(fpEmail)) return setFpError('Please enter a valid email address.');
  ...
  // ── FORGOT: resend OTP → real API ────────────────────────────────────────
  const handleFpResend = async () => {
    if (resendTimer > 0) return;
    try {
      await resendOtp({ email: fpEmail, purpose: 'password_reset' });
      setFpSuccess('OTP resent!'); setTimeout(() => setFpSuccess(''), 2000);
      startResendTimer();
      setOtp(['','','','','','']);
    } catch (err) {
      setFpError(err.message);
  ...
  // ── FORGOT: step 2 — verify OTP → real API ───────────────────────────────
  const handleFpVerifyOtp = async () => {
    if (otp.join('').length < 6) return setFpError('Please enter the complete 6-digit OTP.');
    setFpLoading(true); setFpError('');
    try {
      await verifyResetOtp({ email: fpEmail, otp: otp.join('') });
      setFpStep(3);
    } catch (err) {
      setFpError(err.message);
    } finally {
  ...
  // ── FORGOT: step 3 — reset password → real API ───────────────────────────
  const handleFpReset = async () => {
    if (!newPassword || !confirmPassword) return setFpError('Please fill in both fields.');
  ...
  const openForgot = () => {
    setShowForgot(true);
    setFpStep(1); setFpEmail(''); setFpError(''); setFpSuccess('');
    setOtp(['','','','','','']); setNewPassword(''); setConfirmPassword('');
  };
      ...
      <div style={{ minHeight: '100vh', background: '#EEF2F7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito', sans-serif", padding: '2rem' }}>
        <div style={{ background: '#fff', borderRadius: '24px', boxShadow: '0 8px 40px rgba(27,43,75,0.10)', width: '100%', maxWidth: '420px', padding: '2.5rem 2.2rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1.8rem', width: '100%', maxWidth: '260px' }}>
            {[1,2,3].map((s, i) => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 'none' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: fpStep >= s ? '#1B2B4B' : '#E8EDF5', color: fpStep >= s ? '#fff' : '#8898A9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '800', flexShrink: 0, border: fpStep === s ? '2px solid #F5A623' : '2px solid transparent', transition: 'all 0.3s' }}>
                  {fpStep > s ? '✓' : s}
                </div>
                {i < 2 && <div style={{ flex: 1, height: '2px', margin: '0 4px', background: fpStep > s ? '#1B2B4B' : '#E8EDF5', transition: 'background 0.3s' }} />}
              </div>
            ))}
          </div>

          <h2 style={{ fontSize: '21px', fontWeight: '800', color: '#1B2B4B', marginBottom: '0.3rem' }}>{stepTitles[fpStep - 1]}</h2>
          <p style={{ fontSize: '13px', color: '#8898A9', fontWeight: '600', marginBottom: '1.8rem', textAlign: 'center' }}>{stepSubs[fpStep - 1]}</p>

          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {fpError   && <Banner type="error"   message={fpError} />}
            {fpSuccess && <Banner type="success" message={fpSuccess} />}
            ...
            {fpStep === 1 && (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={labelStyle}>Student Email</label>
                  <div style={{ position: 'relative' }}>
                    <input type="email" placeholder="student@college.edu" value={fpEmail} onChange={e => setFpEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleFpSendOtp()} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                    <span style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#8898A9' }}>📧</span>
                  </div>
                </div>
                <PrimaryBtn onClick={handleFpSendOtp} loading={fpLoading}>{fpLoading ? 'Sending…' : 'Send OTP'}</PrimaryBtn>
              </>
            )}
              ...
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <div style={{ textAlign: 'center', marginTop: '4px' }}>
                    {resendTimer > 0
                      ? <span style={{ fontSize: '12px', color: '#8898A9', fontWeight: '600' }}>Resend OTP in <span style={{ color: '#F5A623', fontWeight: '800' }}>{resendTimer}s</span></span>
                      : <span onClick={handleFpResend} style={{ fontSize: '12px', color: '#F5A623', fontWeight: '700', cursor: 'pointer' }}>Resend OTP</span>
                    }
                  </div>
                </div>
                <PrimaryBtn onClick={handleFpVerifyOtp} loading={fpLoading}>{fpLoading ? 'Verifying…' : 'Verify OTP'}</PrimaryBtn>
                <button onClick={() => { setFpStep(1); setOtp(['','','','','','']); setFpError(''); }} style={{ background: 'none', border: 'none', color: '#8898A9', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", textAlign: 'center' }}>← Change Email</button>
              </>
            )}
            ...
            {fpStep === 3 && (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={labelStyle}>New Password</label>
                  <div style={{ position: 'relative' }}>
                    <input type={showNew ? 'text' : 'password'} placeholder="Min. 6 characters" value={newPassword} onChange={e => setNewPassword(e.target.value)} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                    <span onClick={() => setShowNew(v => !v)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#8898A9', cursor: 'pointer', userSelect: 'none' }}>{showNew ? '🙈' : '👁️'}</span>
                  </div>
                </div>
                {newPassword && (
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '-6px' }}>
                    {[1,2,3,4].map(i => <div key={i} style={{ flex: 1, height: '4px', borderRadius: '4px', background: newPassword.length >= i * 3 ? (newPassword.length >= 10 ? '#27A86E' : newPassword.length >= 6 ? '#F5A623' : '#E74C3C') : '#E8EDF5', transition: 'background 0.2s' }} />)}
                    <span style={{ fontSize: '11px', fontWeight: '700', color: newPassword.length >= 10 ? '#27A86E' : newPassword.length >= 6 ? '#F5A623' : '#E74C3C' }}>
                      {newPassword.length >= 10 ? 'Strong' : newPassword.length >= 6 ? 'Fair' : 'Weak'}
                    </span>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={labelStyle}>Confirm Password</label>
                  <div style={{ position: 'relative' }}>
                    <input type={showConfirm ? 'text' : 'password'} placeholder="Re-enter new password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleFpReset()} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                    <span onClick={() => setShowConfirm(v => !v)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#8898A9', cursor: 'pointer', userSelect: 'none' }}>{showConfirm ? '🙈' : '👁️'}</span>
                  </div>
                  {confirmPassword && (
                    <span style={{ fontSize: '11px', fontWeight: '700', color: newPassword === confirmPassword ? '#27A86E' : '#E74C3C' }}>
                      {newPassword === confirmPassword ? '✓ Passwords match' : '✗ Passwords do not match'}
                ...
                <PrimaryBtn onClick={handleFpReset} loading={fpLoading}>{fpLoading ? 'Resetting…' : 'Reset Password'}</PrimaryBtn>
              </>
            )}

            <Divider label="or go back" />
            <OutlineButton onClick={() => setShowForgot(false)}>← Back to Sign In</OutlineButton>
          </div>
          <FooterDots />
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: '#EEF2F7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito', sans-serif", padding: '2rem' }}>
      <div style={{ background: '#fff', borderRadius: '24px', boxShadow: '0 8px 40px rgba(27,43,75,0.10)', width: '100%', maxWidth: '420px', padding: '2.5rem 2.2rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {/* If unverified, nudge them to complete signup */}
          {needsVerification && (
            <button onClick={() => navigate('/student/signup')}
              style={{ width: '100%', padding: '11px', background: '#FFF4E0', border: '1.5px solid #F5A623', borderRadius: '12px', color: '#B87A00', fontSize: '13px', fontWeight: '700', fontFamily: "'Nunito', sans-serif", cursor: 'pointer' }}>
              ✉️ Complete email verification →
            </button>
          )}
          ...
          <PrimaryBtn onClick={handleLogin} loading={loading}>{loading ? 'Signing in…' : 'Sign In as Student'}</PrimaryBtn>
          ...
          <Divider label="or go back" />
          <OutlineButton onClick={() => navigate('/')}>← Back to Role Selection</OutlineButton>
        </div>
...
function FooterDots() {
  return <div style={{ display: 'flex', gap: '8px', marginTop: '1.8rem' }}>{['#1B2B4B','#4D7BA3','#F5A623','#B8C8DA'].map((c,i) => <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: c }} />)}</div>;
}
function Banner({ type, message }) {
  const isErr = type === 'error';
  return <div style={{ background: isErr ? '#FFF0F0' : '#E3F5EE', border: `1.5px solid ${isErr ? '#F5C6C6' : '#A8DFC8'}`, borderRadius: '10px', padding: '10px 14px', fontSize: '13px', color: isErr ? '#C0392B' : '#1A7A50', fontWeight: '700' }}>{isErr ? '⚠️' : '✅'} {message}</div>;
}
function PrimaryBtn({ onClick, children, loading }) {
  return (
    <button onClick={onClick} disabled={loading}
      ...
      onMouseDown={e => e.currentTarget.style.transform = 'scale(0.98)'}
      onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}
      style={{ width: '100%', padding: '13px 0', background: '#1B2B4B', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '800', fontFamily: "'Nunito', sans-serif", cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.18s, transform 0.12s', opacity: loading ? 0.7 : 1 }}>
      {children}
    </button>
  );
}
