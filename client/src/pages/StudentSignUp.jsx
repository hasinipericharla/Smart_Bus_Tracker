...
import { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
// import { signup, verifyEmail, resendOtp } from '../server/studentAuthService';
import { signup, verifyEmail, resendOtp } from '../api/studentAuthService';
...
const OTP_EXPIRY_SECONDS = 600; // must match backend (10 min)
const RESEND_COOLDOWN    = 30;
...
export default function StudentSignUp() {
  ...
  const handleOtpChange = (index, value) => {
    if (!/^\d*$/.test(value)) return;
    const digits = [...otpDigits]; digits[index] = value.slice(-1); setOtpDigits(digits);
    if (value && index < 5) otpRefs.current[index + 1]?.focus();
  };
  const handleOtpKeyDown = (index, e) => {
  ...
  const handleVerifyOtp = async () => {
    if (timeLeft === 0) return setError('OTP has expired. Please resend.');
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#EEF2F7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito', sans-serif", padding: '2rem' }}>
      <div style={{ background: '#fff', borderRadius: '24px', boxShadow: '0 8px 40px rgba(27,43,75,0.10)', width: '100%', maxWidth: '420px', padding: '2.5rem 2.2rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Logo />
        <Badge>🎓 Student Registration</Badge>

        {/* ── FORM ───────────────────────────────────────────────────────── */}
        {step === 'form' && (
          <>
            <h2 style={{ fontSize: '21px', fontWeight: '800', color: '#1B2B4B', marginBottom: '0.3rem' }}>Create Student Account</h2>
            <p style={{ fontSize: '13px', color: '#8898A9', fontWeight: '600', marginBottom: '1.8rem', textAlign: 'center' }}>Register to start tracking your bus</p>
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {error && <ErrorBanner message={error} />}
              <Field label="Full Name" htmlFor="name">
                <input id="name" type="text" placeholder="Your full name" value={name} onChange={e => setName(e.target.value)} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                <Icon>🪪</Icon>
              </Field>
              <Field label="Email Address" htmlFor="email">
                <input id="email" type="email" placeholder="student@college.edu" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                <Icon>📧</Icon>
              </Field>
              <Field label="Student ID" htmlFor="sid">
                <input id="sid" type="text" placeholder="e.g. STU2024001" value={studentId} onChange={e => setStudentId(e.target.value)} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                <Icon>🎓</Icon>
              </Field>
              <Field label="Password" htmlFor="pw">
                <input id="pw" type={showPassword ? 'text' : 'password'} placeholder="Min. 6 characters" value={password} onChange={e => setPassword(e.target.value)} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                <ToggleIcon show={showPassword} onToggle={() => setShowPassword(v => !v)} />
              </Field>
              {password && (
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '-6px' }}>
                  {[1,2,3].map(i => <div key={i} style={{ flex: 1, height: '4px', borderRadius: '4px', background: i <= strengthLevel ? strengthColors[strengthLevel] : '#E8EDF5', transition: 'background 0.2s' }} />)}
                  <span style={{ fontSize: '11px', fontWeight: '700', color: strengthColors[strengthLevel] }}>{strengthLabels[strengthLevel]}</span>
                </div>
              )}
              <Field label="Confirm Password" htmlFor="confirm">
                <input id="confirm" type={showConfirm ? 'text' : 'password'} placeholder="Re-enter your password" value={confirm} onChange={e => setConfirm(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendOtp()} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                <ToggleIcon show={showConfirm} onToggle={() => setShowConfirm(v => !v)} />
              </Field>
              <PrimaryButton onClick={handleSendOtp} loading={loading} color="#F5A623" hoverColor="#e09400">
                {loading ? 'Sending OTP…' : 'Send Verification OTP'}
              ...
              <Divider label="already have an account?" />
              <OutlineButton onClick={() => navigate('/student/login')}>← Back to Sign In</OutlineButton>
            </div>
          </>
        )}
        ...
        {step === 'otp' && (
          <>
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }} onPaste={handleOtpPaste}>
                {otpDigits.map((digit, i) => (
                  <input key={i} ref={el => otpRefs.current[i] = el}
                    ...
                    onChange={e => handleOtpChange(i, e.target.value)}
                    onKeyDown={e => handleOtpKeyDown(i, e)}
                    style={{ width: '46px', height: '54px', textAlign: 'center', fontSize: '22px', fontWeight: '800', fontFamily: "'Nunito', sans-serif", border: `2px solid ${digit ? '#F5A623' : '#D8E2EE'}`, borderRadius: '12px', outline: 'none', color: '#1B2B4B', background: digit ? '#FFF9EE' : '#F7FAFD', transition: 'all 0.15s' }}
                    onFocus={e => { e.target.style.borderColor = '#F5A623'; e.target.style.boxShadow = '0 0 0 3px rgba(245,166,35,0.15)'; }}
                    onBlur={e => { e.target.style.boxShadow = 'none'; if (!digit) e.target.style.borderColor = '#D8E2EE'; }}
                  />
                ))}
              ...
              <PrimaryButton onClick={handleVerifyOtp} loading={loading} color="#F5A623" hoverColor="#e09400">
              ...
              <OutlineButton onClick={() => { setStep('form'); setError(''); }}>← Change Email</OutlineButton>
            </div>
          </>
        )}
...
function Badge({ children }) {
  return <div style={{ background: '#FFF4E0', color: '#B87A00', borderRadius: '50px', padding: '6px 18px', fontSize: '12px', fontWeight: '800', letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: '1.2rem', display: 'flex', alignItems: 'center', gap: '6px' }}>{children}</div>;
}
function FooterDots() {
...
function Field({ label, htmlFor, children }) {
  return <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}><label htmlFor={htmlFor} style={labelStyle}>{label}</label><div style={{ position: 'relative' }}>{children}</div></div>;
}
function Icon({ children }) {
  return <span style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#8898A9', pointerEvents: 'none' }}>{children}</span>;
}
function ToggleIcon({ show, onToggle }) {
  return <span onClick={onToggle} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#8898A9', cursor: 'pointer', userSelect: 'none' }}>{show ? '🙈' : '👁️'}</span>;
}
function ErrorBanner({ message }) {
  return <div style={{ background: '#FFF0F0', border: '1.5px solid #F5C6C6', borderRadius: '10px', padding: '10px 14px', fontSize: '13px', color: '#C0392B', fontWeight: '700' }}>⚠️ {message}</div>;
}
function PrimaryButton({ onClick, children, loading, color = '#1B2B4B', hoverColor = '#F5A623' }) {
  return (
    <button onClick={onClick} disabled={loading}
      ...
      onMouseDown={e => e.currentTarget.style.transform = 'scale(0.98)'}
      onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}
      style={{ width: '100%', padding: '13px 0', marginTop: '4px', background: color, color: '#fff', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '800', fontFamily: "'Nunito', sans-serif", cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.18s, transform 0.12s', opacity: loading ? 0.7 : 1 }}>
      {children}
    </button>
  );
}
